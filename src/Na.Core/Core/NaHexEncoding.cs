...
namespace Na.Core
{
	...
	/// </summary>
	public static class NaHexEncoding
	{
		/// <summary>
		...
		public static string BinToHex(ReadOnlySpan<byte> bin)
		{
			...
			Span<byte> hexAsciiBytes = hexAsciiBytesLen <= NaConstants.MaxStackAlloc ? stackalloc byte[hexAsciiBytesLen] : new byte[hexAsciiBytesLen];
			var result = Libsodium.sodium_bin2hex(hexAsciiBytes, (nuint)hexAsciiBytesLen, bin, (nuint)bin.Length);
			if (result == 0)
			{
				throw new NaException("sodium_bin2hex failed");
			}
		...
		public static Span<byte> HexToBin(ReadOnlySpan<char> hex, Span<byte> bin, string? ignore = null)
		{
			...
			Span<byte> hexBytes = hex.Length <= NaConstants.MaxStackAlloc ? stackalloc byte[hex.Length] : new byte[hex.Length];
			...
			{
				throw new NaException("sodium_hex2bin failed because hex string contains invalid characters or the destination bin buffer is too short");
			}
