...
namespace Na.Core
{
	...
	public static class NaBase64Encoding
	{
		...
		public static Span<byte> Base64ToBin(ReadOnlySpan<char> b64, Span<byte> bin, Base64Variant variant, string? ignore = null)
		{
			...
			Span<byte> b64AsciiBytes = b64.Length <= NaConstants.MaxStackAlloc ? stackalloc byte[b64.Length] : new byte[b64.Length];
			...
			{
				throw new NaException("sodium_base642bin failed because Base64 contains invalid characters or the destination bin buffer is too short");
			}
		...
		public static string BinToBase64(ReadOnlySpan<byte> bin, Base64Variant variant)
		{
			...
			Span<byte> b64AsciiBytes = b64AsciiBytesLen <= NaConstants.MaxStackAlloc ? stackalloc byte[b64AsciiBytesLen] : new byte[b64AsciiBytesLen];
			var result = Libsodium.sodium_bin2base64(b64AsciiBytes, (nuint)b64AsciiBytes.Length, bin, (nuint)bin.Length, (int)variant);
			if (result == 0)
			{
				throw new NaException("sodium_bin2base64 failed");
			}
