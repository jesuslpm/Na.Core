...
namespace Na.Core
{
	...
	/// </summary>
	public static partial class NaSecretBox
	{
		// KeyLen represents the length of the encryption key in bytes.
		public const int KeyLen = Libsodium.crypto_secretbox_KEYBYTES;
		// NonceLen represents the length of the nonce (number used once) in bytes.
		public const int NonceLen = Libsodium.crypto_secretbox_NONCEBYTES;
		// MacLen represents the length of the Message Authentication Code (MAC) in bytes.
		public const int MacLen = Libsodium.crypto_secretbox_MACBYTES;
		...
		/// </list>
		/// </exception>
		/// <exception cref="NaException">Thrown when the underlying encryption operation fails.</exception>
		public static Span<byte> EncryptCombined(Span<byte> ciphertext, ReadOnlySpan<byte> plaintext, ReadOnlySpan<byte> key, ReadOnlySpan<byte> nonce)
		{
			...
			int rc = Libsodium.crypto_secretbox_easy(ciphertext, plaintext, (ulong)plaintext.Length, nonce, key);
			if (rc != 0)
			{
				throw new NaException("Failed to encrypt message.");
			}
		...
		public static Span<byte> EncryptCombined(Span<byte> ciphertext, ReadOnlySpan<byte> plaintext, ReadOnlySpan<byte> key)
		{
			...
			int rc = Libsodium.crypto_secretbox_easy(cipher, plaintext, (ulong)plaintext.Length, nonce, key);
			if (rc != 0)
			{
				throw new NaException("Failed to encrypt message.");
			}
		...
		/// </list>
		/// </exception>
		/// <exception cref="NaException">Thrown when decryption or verification fails.</exception>
		...
		public static Span<byte> DecryptCombined(Span<byte> plaintext, ReadOnlySpan<byte> ciphertext, ReadOnlySpan<byte> key, ReadOnlySpan<byte> nonce)
		{
			...
			NaInitializer.EnsureInitialized();
			int rc = Libsodium.crypto_secretbox_open_easy(plaintext, ciphertext, (ulong)ciphertext.Length, nonce, key);
			if (rc != 0)
			{
				throw new NaException("Couldn't decrypt message. Verification failed");
			}
		...
		/// </list>
		/// </exception>
		/// <exception cref="NaException">Thrown when the encryption operation fails.</exception>
		public static Span<byte> EncryptDetached(Span<byte> ciphertext, Span<byte> mac, ReadOnlySpan<byte> plaintext, ReadOnlySpan<byte> key, ReadOnlySpan<byte> nonce)
		{
			...
			NaInitializer.EnsureInitialized();
			int rc = Libsodium.crypto_secretbox_detached(ciphertext, mac, plaintext, (ulong)plaintext.Length, nonce, key);
			if (rc != 0)
			{
				throw new NaException("Failed to encrypt message.");
			}
		...
		public static Span<byte> EncryptDetached(Span<byte> ciphertext, Span<byte> mac, ReadOnlySpan<byte> plaintext, ReadOnlySpan<byte> key)
		{
			...
			NaRandom.Fill(nonce);
			var cipher = ciphertext.Slice(NonceLen);
			int rc = Libsodium.crypto_secretbox_detached(cipher, mac, plaintext, (ulong)plaintext.Length, nonce, key);
			if (rc != 0)
			{
				throw new NaException("Failed to encrypt message.");
			}
		...
		public static Span<byte> DecryptDetached(Span<byte> plaintext, ReadOnlySpan<byte> ciphertext, ReadOnlySpan<byte> key, ReadOnlySpan<byte> mac, ReadOnlySpan<byte> nonce)
		{
			...
			NaInitializer.EnsureInitialized();
			int rc = Libsodium.crypto_secretbox_open_detached(plaintext, ciphertext, mac, (ulong)ciphertext.Length, nonce, key);
			if (rc != 0)
			{
				throw new NaException("Couldn't decrypt message. Verification failed");
			}
		...
		public static Span<byte> DecryptDetached(Span<byte> plaintext, ReadOnlySpan<byte> ciphertext, ReadOnlySpan<byte> key, ReadOnlySpan<byte> mac)
		{
			...
			var cipher = ciphertext.Slice(NonceLen);
			NaInitializer.EnsureInitialized();
			int rc = Libsodium.crypto_secretbox_open_detached(plaintext, cipher, mac, (ulong)cipher.Length, nonce, key);
			if (rc != 0)
			{
				throw new NaException("Couldn't decrypt message. Verification failed");
			}
