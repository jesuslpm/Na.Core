...
namespace Na.Core
{
	...
	/// </summary>
	public static partial class NaSecureMemory
	{
		/// <summary>
		...
		/// <exception cref="NaException">Thrown if locking the memory fails.</exception>
		public static void MemLock(Span<byte> buf)
		{
			NaInitializer.EnsureInitialized();
			if (Libsodium.sodium_mlock(buf, (nuint)buf.Length) != 0)
			{
				throw new NaException("Failed to lock memory");
			}
		}
		...
		/// <exception cref="NaException">Thrown if unlocking the memory fails.</exception>
		...
		public static void MemUnlock(Span<byte> buf)
		{
			...
			{
				throw new NaException("Failed to unlock memory");
			}
		}
		...
		/// <exception cref="NaException">Thrown if memory allocation fails.</exception>
		public static Span<byte> Malloc(int size)
		{
			...
			unsafe
			{
				...
				if (ptr == null)
				{
					throw new NaException("Failed to allocate memory");
				}
		...
		/// <exception cref="NaException">Thrown if setting the memory protection fails.</exception>
		public static void ProtectReadOnly(Span<byte> buf)
		{
			NaInitializer.EnsureInitialized();
			if (Libsodium.sodium_mprotect_readonly(buf) != 0)
			{
				throw new NaException("Failed to set memory to read-only");
			}
		}
		...
		public static void ProtectReadWrite(Span<byte> buf)
		{
			...
			{
				throw new NaException("Failed to set memory to read-write");
			}
		}
