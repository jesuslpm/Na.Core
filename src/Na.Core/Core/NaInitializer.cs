...
namespace Na.Core
{
	...
	/// </summary>
	public static partial class NaInitializer
	{
		...
		private static void SodiumInit()
		{
			...
			{
				throw new NaException("Failed to initialize libsodium."); // Throw exception on failure.
			};
		}
		...
		private static void SetMisuseHandler(Action handler)
		{
			...
			{
				throw new NaException("Failed to set misuse handler."); // Throw exception on failure.
			};
		}
		...
		private static void InitializeBindings()
		{
			try
			{
				...
				if (NaVersion.GetMajor() != Libsodium.LIBSODIUM_VERSION_MAJOR ||
					NaVersion.GetMinor() != Libsodium.LIBSODIUM_VERSION_MINOR)
				{
					string? version = NaVersion.GetString();
					throw version != null && version != Libsodium.SODIUM_VERSION_STRING
						? new NaException($"An error occurred while initializing cryptographic primitives. (Expected libsodium {Libsodium.SODIUM_VERSION_STRING} but found {version}.)")
						: new NaException("An error occurred while initializing cryptographic primitives: version mismatch");
				}
		...
		{
			throw new NaException("Misuse detected"); // Throw exception on misuse.
		}
	}
}